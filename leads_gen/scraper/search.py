...
from leads_gen.scraper.zooming import enable_update_results_checkbox
from leads_gen.utils.wait_utils import SmartWait

logger = logging.getLogger("leads_gen")


def set_browser_zoom(driver, zoom_percent: int = 67):
...
def search_maps(driver, query):
    try:
        ...
        # Try to handle any consent/cookie dialogs. Google occasionally A/B tests new
        # dialogs; if we can't find or dismiss it, we log and press on rather than
        ...
        try:
            ...
            logger.debug(f"Cookie dialog present but not dismissable: {e}")
        except Exception as e:
            # Anything else is unexpected — log at WARNING with traceback so we notice.
            logger.warning("Unexpected error handling cookie dialog: %s", e, exc_info=True)
        ...
        for by_type, selector in selectors:
            ...
            search_box = smart_wait.wait_for_element(
                by_type, selector, timeout=timeout, condition="visibility"
            )
            if search_box:
        ...
        results_loaded = smart_wait.wait_for_element(
            By.XPATH, '//div[@role="feed"]', timeout=results_timeout, condition="presence"
        )

        if results_loaded:
            logger.info(f'Search for "{query}" completed - results loaded')
        else:
            logger.warning("Search results may not have loaded properly")
