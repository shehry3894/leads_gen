...
from leads_gen.licensing.license_codec import decode_license
...
class LicenseManager:
    ...
    def __init__(self, license_file: str = "license.key"):
        ...
        self.license_data: LicenseData | None = None
    ...
    def save_license_to_file(self, license_key: str | None = None) -> bool:
        """
    ...
    def can_scrape(self, requested_results: int) -> tuple[bool, str]:
        ...
        if requested_results > max_allowed:
            return (
                False,
                f"Requested {requested_results} results, but license allows maximum {max_allowed}",
            )
    ...
    def get_license_info(self) -> dict:
        ...
        """
        if not self.license_data:
            return {"valid": False, "status": "No license", "message": "No valid license key found"}

        is_trial = (self.license_data.features or {}).get("trial", False)
        days_remaining = self.license_data.days_remaining()
        expired = self.license_data.is_expired()
        fingerprint_ok = self.license_data.matches_fingerprint(self.machine_fingerprint)

        return {
            "valid": (not expired) and fingerprint_ok,
    ...
    def initialize(self) -> tuple[bool, str]:
        ...
        if self.load_license_from_file():
            ...
            if is_valid:
                ...
                return (
                    True,
                    f"License active: {info['type']} ({info['days_remaining']} days remaining)",
                )
            return False, error
